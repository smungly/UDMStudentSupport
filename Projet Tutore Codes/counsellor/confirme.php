...
*/
  ...
  $sql = "SELECT DISTINCT membres.EMAIL,membres.ID_MEMBRE,membres.NOM, membres.PRENOM, rdv.* FROM membres, rdv WHERE rdv.ID_MEMBRE = membres.ID_MEMBRE AND rdv.STATUT = 'Confirmes'";
      ...
      $dayOfWeek = 'VENDREDI';
        ...
        echo  '<div class="card">';
            ...
            echo '<form method = "POST" action="reminder.php">';
                            ...
                            echo '<input type="hidden" id="date_ajd" name="date_ajd" value='.$dayOfWeek.'>';
                            echo '<input type="hidden" id="hd" name="hd" value='.$hd.'>';
                            echo '<input type="hidden" id="hf" name="hf" value='.$hf.'>';
                            echo '<input type="hidden" id="email" name="email" value='.$email.'>';
                            echo '<p style="text-align: right"><small style="color: #66b266!important" id="help" class="form-text text-muted"><i>*Envoyer un mail de Rappel à l\'étudiant</i></small></p>';
                            echo '<p style="text-align: right"><button type="submit" class="btn btn-outline-light" data-toggle="tooltip" data-placement="bottom" title="Notifier l\'étudiant">&#128276;</button></p>';
            ...
            echo '<form method="POST" action="modif.php">';
               ...
               echo '</div>';
                  ...
                  echo '</div>';
                    echo '<textarea style="height: 45px;" class="form-control" id="des" aria-label="Description" readonly>'.$des.'</textarea>';
                echo '</div>';
              ...
              echo '<div class="form-group">';
               ...
               echo '<p style="text-align: right"><button type="submit" class="btn btn-outline-secondary">Sauvegarder</button></p>';
